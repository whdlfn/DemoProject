...
public class BookCaseSaveController implements Controller {
	...
	public void handle(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//서버의 로컬 경로에 대해 url 지정 - server.xml
//	      <Context docBase="D:\\Class231228\\upload" path="/upload"/>

		// 업로드 가능한 최대 크기(바이트)
		int maxSize = 10 * 1024 * 1024;

		// request 는 파일을 받을 수 있는 MultipartRequest로 사용해야 합니다. : cos 라이브러리 추가
		MultipartRequest multipartRequest = new MultipartRequest(request, path, maxSize, "UTF-8",
				new DefaultFileRenamePolicy());
		// DefaultFileRenamePolicy 는 중복되 파일명에 대해 새로파일명 부여하는 규칙(파일명 뒤에 일련번호)
		...
		// 서버의 파일 시스텐 D:\\Class231228\\upload 경로에서 파일이 생겼는지 확인하기
...
//		log.info(":::파일명-{}:::",cover); 	//uploadFile
		...
		try {
			...
			if (result != 1) {
				// todo : 이전 예제 참고해서 등록 실패하면 쿠기이용해서 메시지를 전달 해 주세요
				url = "new";
			}
		} catch (Exception e) {
		}
